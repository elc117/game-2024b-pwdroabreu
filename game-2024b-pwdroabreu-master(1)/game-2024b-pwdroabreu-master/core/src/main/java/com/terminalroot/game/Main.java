...
public class Main extends Game {
    ...
    public void create() {
        ...
        float y = (screenHeight / 2) - (objectHeight / 2);

        // Criar e posicionar os objetos
        for (int i = 0; i < numberOfObjects; i++) {
            float x = startX + i * (objectWidth + spacing); // Posiciona cada objeto
            InteractiveObject obj = new InteractiveObject(objectTexture, x, y);
            obj.setSize(objectWidth, objectHeight); // Redimensiona o objeto
            objects.add(obj);
        }
    }
    ...
    public void render() {
        ...
        batch.begin();
        batch.draw(background, 0, 0); // Desenha o fundo primeiro
        for (InteractiveObject obj : objects) {
            obj.draw(batch); // Desenha os objetos interativos
        }
        dinoSprite.setPosition(dinoX, dinoY);
        dinoSprite.draw(batch); // Desenha o dinossauro por cima dos objetos
        batch.end();
    }
